...
class TimerCountdown extends StatefulWidget {
  ...
  /// Defines seperator list.
  TimerSperatorSignModel? speratorSignModel = TimerSperatorSignModel();

  ///time with unit flag
  bool isWithUnit = false;

  ///Fill in the gap with 0
  bool isNeedFullZero = true;
  ...
  TimerCountdown({
    ...
    this.spacerWidth = 10,
    this.speratorSignModel,
    this.isWithUnit = false,
    this.isNeedFullZero = true,
  });
...
class _TimerCountdownState extends State<TimerCountdown> {
  ...
  String _twoDigits(int n, String unitType) {
    switch (unitType) {
      case "minutes":
        ...
        if (n >= 10) return "$n${timeTypeUnit(CountDownTimerValueType.minute)}";
        return "${fullGapChar()}$n${timeTypeUnit(CountDownTimerValueType.minute)}";
      case "hours":
        ...
        if (n >= 10) return "$n${timeTypeUnit(CountDownTimerValueType.hour)}";
        return "${fullGapChar()}$n${timeTypeUnit(CountDownTimerValueType.hour)}";
      case "days":
        ...
        if (n >= 10) return "$n${timeTypeUnit(CountDownTimerValueType.day)}";
        return "${fullGapChar()}$n${timeTypeUnit(CountDownTimerValueType.day)}";
      default:
        if (n >= 10) return "$n${timeTypeUnit(CountDownTimerValueType.sencod)}";
        return "${fullGapChar()}$n${timeTypeUnit(CountDownTimerValueType.sencod)}";
    }
  }
    ...
    switch (widget.format) {
      case CountDownTimerFormat.daysHoursMinutesSeconds:
        return Row(
          ...
          children: [
            _days(context),
            _colon(widget.speratorSignModel!.dayHour),
            _hours(context),
            _colon(widget.speratorSignModel!.hourMinute),
            _minutes(context),
            _colon(widget.speratorSignModel!.minuteSecode),
            _seconds(context),
          ],
        );
  ...
  String fullGapChar(){
    if(widget.isNeedFullZero){
      return "0";
    }
    return "";
  }

  String timeTypeUnit(CountDownTimerValueType type){
    if(widget.isWithUnit){
      switch(type){

        case CountDownTimerValueType.day:{
          return "d";
        }
          break;
        case CountDownTimerValueType.hour:{
          return "h";
        }
        break;
        case CountDownTimerValueType.sencod:{
          return "s";
        }
        break;
        case CountDownTimerValueType.minute:{
          return "m";
        }
        break;
